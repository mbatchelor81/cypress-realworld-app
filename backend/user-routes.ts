...
router.get("/", ensureAuthenticated, async (req, res) => {
  ...
  const users = removeUserFromResults(req.user?.id!, await getAllUsers()); // NOSONAR: non-null assertion needed; user guaranteed by ensureAuthenticated
...
router.get("/search", ensureAuthenticated, validateMiddleware([searchValidation]), async (req, res) => {
  ...
  /* istanbul ignore next */
  const users = removeUserFromResults(req.user?.id!, await searchUsers(q as string)); // NOSONAR
