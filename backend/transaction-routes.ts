...
router.get(
  ...
  async (req, res) => {
    ...
    const transactions = await getTransactionsForUserForApi(req.user?.id!, req.query); // NOSONAR
...
router.get(
  ...
  async (req, res) => {
    /* istanbul ignore next */
    const transactions = await getTransactionsForUserContacts(req.user?.id!, req.query); // NOSONAR
...
router.get(
  ...
  async (req, res) => {
    ...
    let transactions = !isEmpty(req.query)
      ? await getPublicTransactionsByQuery(req.user?.id!, req.query) // NOSONAR
      : /* istanbul ignore next */
        await getPublicTransactionsDefaultSort(req.user?.id!); // NOSONAR
...
router.post(
  ...
  async (req, res) => {
    ...
    /* istanbul ignore next */
    const transaction = await createTransaction(req.user?.id!, transactionType, transactionPayload); // NOSONAR
