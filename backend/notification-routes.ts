...
router.get("/", ensureAuthenticated, async (req, res) => {
  ...
  const notifications = await getUnreadNotificationsByUserId(req.user?.id as string);
...
router.post(
  ...
  async (req, res) => {
    ...
    /* istanbul ignore next */
    const notifications = await createNotifications(req.user?.id as string, items);
...
router.patch(
  ...
  async (req, res) => {
    ...
    /* istanbul ignore next */
    await updateNotificationById(req.user?.id as string, notificationId, req.body);
