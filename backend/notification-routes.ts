...
router.get("/", ensureAuthenticated, async (req, res) => {
  ...
  const notifications = await getUnreadNotificationsByUserId(req.user?.id!); // NOSONAR
...
router.post(
  ...
  async (req, res) => {
    ...
    /* istanbul ignore next */
    const notifications = await createNotifications(req.user?.id!, items); // NOSONAR
...
router.patch(
  ...
  async (req, res) => {
    ...
    /* istanbul ignore next */
    await updateNotificationById(req.user?.id!, notificationId, req.body); // NOSONAR
