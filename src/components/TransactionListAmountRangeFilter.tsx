...
type AmountRangeFilterProps = {
  ...
  handleAmountRangeChange: (_event: Event | React.SyntheticEvent, amountRange: number | number[]) => void;
};
...
const TransactionListAmountRangeFilter: React.FC<TransactionListAmountRangeFilterProps> = ({
  ...
  const amountRangeFilterProps: AmountRangeFilterProps = {
    amountRangeValue,
    initialAmountRange,
    setAmountRangeValue,
    resetAmountRange,
  ...
  return (
    <div>
      ...
      {!xsBreakpoint && (
        ...
        >
          <AmountRangeFilter {...amountRangeFilterProps} />
        </Popover>
      )}
      {xsBreakpoint && (
        <Drawer
